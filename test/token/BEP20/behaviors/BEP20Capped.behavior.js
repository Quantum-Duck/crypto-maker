...
function shouldBehaveLikeBEP20Capped (cap, [minter, other]) {
  describe('totalSupply', function () {
    ...
    it('should fail to mint if the amount exceeds the cap', async function () {
      await this.token.mint(other, cap.subn(1), { from });
      await expectRevert(this.token.mint(other, 2, { from }), 'BEP20Capped: cap exceeded');
    });

    it('should fail to mint after cap is reached', async function () {
      await this.token.mint(other, cap, { from });
      await expectRevert(this.token.mint(other, 1, { from }), 'BEP20Capped: cap exceeded');
    });
  });
}

module.exports = {
  shouldBehaveLikeBEP20Capped,
};